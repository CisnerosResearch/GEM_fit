...
c      leading zeros in place. E.g. V0003.22. Leading 0s should also
...
c   subsequent calls we will know immediately if we should read further
...
c   line exists. If not, then we assume it is an old format PARM file. In
...
c
c   If this is an old-style PARM file, we cannot do the search. Simply
