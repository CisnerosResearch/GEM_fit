...
C
        ...
        IF(ITITL1(1).EQ.IIEND) then
          write(stdout, '(4x,a)') '----- END OF GROUP READ -----'
          GO TO 900
        endif
        NGRP = NGRP+1
        write(stdout,'(4x,a,i5,a)') '----- READING GROUP ',
     +                       ngrp, '; TITLE:'
        write(stdout,9218) (ITITL1(K),K=1,19)
C
...
C
          ...
          DO 64 III = 1,12
            ...
            write(stdout,202) JGRAPH(III),JSYMBL(III),JTREE(III),JRESNM(III)
   64     CONTINUE
   65     CONTINUE
C
          ISRCH = III-1
          IF(ISRCH.GT.10) write(stdout,66) ISRCH
C
...
C
          ...
          DO 12 I = 1,7
            ...
            IF(I1.LT.0) I1 = -I1
            IF(I2.LE.0) I2 = I1
            IF(LSIGN.EQ.0) write(stdout,14) NGRP,I1,I2
            DO 13 J = I1,I2
              ISTART = IPRES(J)
              IEND = IPRES(J+1)-1
              DO 45 K = ISTART,IEND
     ...
     +                       ITREE,IGRAPH)
                ...
                NATMG = NATMG+1
   45         CONTINUE
              IF(LSIGN.EQ.1) write(stdout,46) NGRP,J
...
c       IF(ISRCH.GT.0) write(stdout,199)
        write(stdout,222) NATMG
      GO TO 22
C
   36 CONTINUE
      write(stdout,127) KTYPG,(IGRP(I),JGRP(I),I = 1,7)
      GO TO 10
C
...
C
        ...
        DO 940 I = 1,NATC
          J = IGROUP(I)
          J3 = 3*J-3
          write(stdout,9118) I,J,IGRAPH(J),ISYMBL(J),ITREE(J),
...
C
  ...
  820   CONTINUE
        IF(MISC) NGRP = NGRP+1
c       IF(MISC) write(stdout,9308) NGRP
      endif
