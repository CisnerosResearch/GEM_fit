...
c
      ...
      if (irc .ne. 0) then
         write(stdout,'(/2x,a,i6)') 'Error on fileinf: ',irc
         call mexit(6, 1)
      endif
#endif
c
...
c          --- creating new database (PREP only) ---
     ...
     .          iostat=ios)
           ...
           if (ios .ne. 0) then
                write(stdout,'(/2x,a,a)') 'Error on open: ',namf
                call mexit(6, 1)
           endif
           IRECST = IRECST + 4
...
c            --- old dbase - appending by prep ---
     ...
     +          FORM='UNFORMATTED', recl=lenrec, iostat=ios)
             if (ios .ne. 0) then
                write(stdout,'(/2x,a,a)') 'Error on dbase open: ', namf
                call mexit(6, 1)
             endif
          else
c             --- link - read_only ---
...
c             --- link uses unit # ---
     ...
     +          readonly)
              if (ios .ne. 0) then
                write(stdout,'(/2x,a,i6)') 'Error on open: unit ',idaf
                write(stdout,'(20x,a)') '(Unit must be assigned for VMS)'
                call mexit(6, 1)
              endif
#else
              OPEN(UNIT=IDAF, FILE=NAMF, STATUS='OLD', ACCESS='DIRECT',
     +          FORM='UNFORMATTED', recl=lenrec, iostat=ios)
      ...
      call fileinf(irc)
      if (irc .ne. 0) then
         write(stdout,'(/2x,a,i6)') 'Error on fileinf reset: ',irc
         call mexit(6, 1)
      endif
#endif
      RETURN
