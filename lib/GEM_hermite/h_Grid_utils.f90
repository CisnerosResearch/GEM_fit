...
! These subroutintes generously provided by Paul M. Ayers
!
...
!
! kinds -- a module defining default variable types for Ayers-written code.  
! Patrick -- a module, written by Patrick Bultinck et al., that is used to read the
!            wfn file and to compute the electron density.
! Pvariables -- a module containing variables used in the Patrick module.
!
!88888888888888
...
!                       Pvariables                                              !
!                                                                               !
! A module containing variables needed by Patrick Bultinck's module.            !
! Streamlined slightly by Paul Ayers for the present limited purpose.           !
!-------------------------------------------------------------------------------!


! this module contains most universal variables
! names are roughly descriptive of contents
! it is advised to have a copy of Becke's paper alongside
module Pvariables

	implicit none

        INTEGER::stepf_iter,n_leb,interpol_nr
        INTEGER::num_orb,num_prim,num_atoms
        INTEGER::nr_of_pts
        INTEGER::nr_gridpts
        INTEGER,DIMENSION(118)::nr_rad_pts
        INTEGER,DIMENSION(:),ALLOCATABLE::centre_assignm,type_assignm
        INTEGER,DIMENSION(:,:),ALLOCATABLE::prim_atom
        INTEGER,DIMENSION(:),ALLOCATABLE::centre_charge
        ...
        REAL*8::coeflim
        REAL*8::radfac
        REAL*8,DIMENSION(:),ALLOCATABLE::r_points
        REAL*8,DIMENSION(:),ALLOCATABLE::occup_nr,exponents
        REAL*8,DIMENSION(:),ALLOCATABLE::dist
        REAL*8,DIMENSION(:),ALLOCATABLE::w_leb,x_leb,y_leb,z_leb
        REAL*8,DIMENSION(:),ALLOCATABLE::r_weights
        REAL*8,DIMENSION(:),ALLOCATABLE::p_matrix
        REAL*8,DIMENSION(:,:),ALLOCATABLE::mo_coeff,centre_coord,mol_grid
        REAL*8,DIMENSION(95)::BraggSlater_radii
        REAL*8,DIMENSION(20,3)::type_exponents

end module Pvariables
...
!                               read_data                                       !
!                                                                               !
! This is a subroutine from Patrick Bultinck that reads *.wfn files.            !
! Slightly modified by Paul Ayers.                                              !
!-------------------------------------------------------------------------------!

subroutine read_data(read_file)

! This subroutine gathers all required/useful data from the wfn file
! num_orb: number of orbitals (only occupied are by default in a wfn file)
! you may want to make your own wfn writer from e.g. a Gaussian fchk file
! to have more detailed info, e.g., including all virtuals as well if you
! would want to project something on the basis of the naturals
! num_prim: number of primitives
! num_atoms: number of atoms
! centre_coord: coordinates of all atoms hence dimension (atom,3)
! centre_charge: array with atomic numbers
! centre_assignm: array linking primitives and atoms
! type_assignm: array containing the type of the primitive, px, py, pz, dzz...
! the different types are defined elsewhere
! exponents: primitive exponents
! mo_coeff: BIG matrix that contains for all MO's all expansion coefficients
! of the primitives
! occup_nr: array with the MO occupation numbers

USE Pvariables

implicit none

INTEGER::istat,i,j,k,l,m,orbital
CHARACTER(80)::line
CHARACTER(30)::read_file

orbital = 0
numel = 0

! open the wfn file
open (UNIT=10,FILE=read_file,STATUS='old',ACCESS='SEQUENTIAL',IOSTAT=istat)

do

        read (10,'(A80)',IOSTAT=istat) line

! When we detect a line that starts with "GAUSSIAN", we know what we can fetch
! Note that for ADF, one would work with STP's and not GTP's, ask for a different version
        IF (line(1:8).EQ.'GAUSSIAN') THEN
        	READ (line,'(18X,I5)') num_orb
                READ(line,'(38X,I5)') num_prim
                READ(line,'(59X,I4)') num_atoms

! Now we can properly allocate many variables
                ALLOCATE(mo_coeff(num_orb,num_prim))
                ALLOCATE(occup_nr(num_orb))
                ALLOCATE(exponents(num_prim))
                ALLOCATE(centre_assignm(num_prim))
                ALLOCATE(type_assignm(num_prim))
                ALLOCATE(centre_coord(num_atoms,3))
                ALLOCATE(centre_charge(num_atoms))

                do l=1,num_atoms
                	READ(10,'(A80)') line
                        READ(line,'(24X,3(F12.8))') (centre_coord(l,m),m=1,3)
! Warning. A wfn file has atomic numbers as reals but we prefer integers.
                        READ(line,'(70X,I3)') centre_charge(l)
                END do

        ENDIF

! centre assignments
        IF (line(1:6).EQ.'CENTRE') THEN
                READ(line,'(20X,20(I3))') (centre_assignm(k),k=1,MIN(20,num_prim))
                do l=2,INT((num_prim-1)/20)+1
                        READ(10,'(20X,20(I3))') (centre_assignm(m),m=(l-1)*20+1,MIN((l-1)*20+20,num_prim))
                end do
        END IF

! type assignments
        IF (line(1:4).EQ.'TYPE') THEN
                READ(line,'(20X,20(I3))') (type_assignm(k),k=1,MIN(20,num_prim))
                do l=2,INT((num_prim-1)/20)+1
                        READ(10,'(20X,20(I3))') (type_assignm(m),m=(l-1)*20+1,MIN((l-1)*20+20,num_prim))
                end do
        END IF

! exponents
        IF (line(1:9).EQ.'EXPONENTS') THEN
                READ(line,'(10X,5(D14.7))') (exponents(k),k=1,MIN(5,num_prim))
                do l=2,INT((num_prim-1)/5)+1
                        READ(10,'(10X,5(D14.7))') (exponents(m),m=(l-1)*5+1,MIN((l-1)*5+5,num_prim))
                end do

        END IF

! MO coefficients
        IF (line(1:2).EQ.'MO') THEN
                orbital = orbital + 1
                READ(line,'(34X,D13.7)') occup_nr(orbital)
                numel=numel+occup_nr(orbital)
        	do i=1, INT((num_prim-1)/5)+1
                	READ (10,'(5(D16.8))') (mo_coeff(orbital,j),j=(i-1)*5+1, MIN((i-1)*5+5,num_prim))
        	END do
...
!                       density_calculation                                     !
!                                                                               !
! This is a subroutine from Patrick Bultinck et al. that computes the molecular !
! density from the *.wfn file  .                                                !
!                                                                               !
! x,y,z are the Cartesian coordinates of the grid point in ***BOHR***.          !
! dens is the electron density.                                                 !
!                                                                               !
! Slightly modified by Paul Ayers.                                              !
!-------------------------------------------------------------------------------!

subroutine density_calculation(x,y,z,dens)

! Compute the molecular density
! given: x,y en z-coordinate
! result: electron density
...
INTEGER::i,j,k
REAL*8::dens,x,y,z,r_dist1,r_dist2,x_p1,y_p1,z_p1,x_p2,y_p2,z_p2
REAL*8::primitive1,primitive2
...
do i = 1,num_prim,1
        ...
        x_p1 = x-centre_coord(centre_assignm(i),1)
        y_p1 = y-centre_coord(centre_assignm(i),2)
        z_p1 = z-centre_coord(centre_assignm(i),3)
        r_dist1 = sqrt(x_p1**2.0+y_p1**2.0+z_p1**2.0)


        CALL primitive(i,r_dist1,x_p1,y_p1,z_p1,primitive1)

	do j = 1,i-1

     		k=k+1
		x_p2 = x-centre_coord(centre_assignm(j),1)
     		y_p2 = y-centre_coord(centre_assignm(j),2)
     		z_p2 = z-centre_coord(centre_assignm(j),3)
 		r_dist2 = sqrt(x_p2**2.0+y_p2**2.0+z_p2**2.0)

	     	CALL primitive(j,r_dist2,x_p2,y_p2,z_p2,primitive2)
	      	dens = dens + (2.0*primitive1*primitive2*p_matrix(k))

     	END do

      k=k+1
      dens = dens + (primitive1**2.0*p_matrix(k))

END do

end subroutine

subroutine primitive(prim_nr,r_coord,x_part,y_part,z_part,prim)
...
INTEGER::prim_nr
REAL*8::prim,x_part,y_part,z_part,r_part,x_pow,y_pow,z_pow,r_coord

x_pow = type_exponents(type_assignm(prim_nr),1)
y_pow = type_exponents(type_assignm(prim_nr),2)
z_pow = type_exponents(type_assignm(prim_nr),3)

r_part = exp(-exponents(prim_nr)*r_coord**2.0)

prim = (x_part**x_pow)*(y_part**y_pow)*(z_part**z_pow)*r_part
...
!                           PREPARE                                             !
!                                                                               !
! This is a subroutine from Patrick Bultinck that generates the Lebedev grids   !
! and writes them to file.   Slightly modified by Paul Ayers.                   !
!-------------------------------------------------------------------------------!


subroutine prepare(rad_grid,wght,npts,maxpts)

	USE Pvariables 
	implicit none

    INTEGER::i,j,k,tot_rad_pts,matrix_size,npts,maxpts
    INTEGER::atom_b
    INTEGER::point_r,point_l,counter
	REAL*8::b_weight
	double precision::rad_grid(maxpts,3),wght(maxpts)

IF (ALLOCATED(w_leb)) deallocate(w_leb)
IF (ALLOCATED(x_leb)) deallocate(x_leb)
IF (ALLOCATED(y_leb)) deallocate(y_leb)
IF (ALLOCATED(z_leb)) deallocate(z_leb)
IF (ALLOCATED(prim_atom)) deallocate(prim_atom)
IF (ALLOCATED(p_matrix)) deallocate(p_matrix)


    ALLOCATE(w_leb(n_leb))
	ALLOCATE(x_leb(n_leb))
	ALLOCATE(y_leb(n_leb))
	ALLOCATE(z_leb(n_leb))
    
    ALLOCATE(prim_atom(num_atoms,2))
    ALLOCATE(p_matrix(num_prim*(num_prim+1)/2))

! The next lines arrange arrays that will be filled with data
! Arranging for Bragg Slater radii
        CALL BraggSlater(BraggSlater_radii)
! Compose the array with angular momentum data
	    CALL ang_momentum_matrix()
! Establish the number of radial points per atom according to Becke
        call nr_radial_points(nr_rad_pts)
! Get the Lebedev grids according to the one chosen
! (THANK YOU Dr. Christoph van Wuellen)
	call lebedev()

! Organize a mapping between primitives and atoms
! of little use except in specific cases
        call atom_primitives()
! Construction of the density matrix from a wfn file

	    call charge_density_matrix()


! Compute the number of points in the grid.
        tot_rad_pts = 0
        do k=1,num_atoms,1
                tot_rad_pts = tot_rad_pts + nr_rad_pts(centre_charge(k))
        end do
        matrix_size = tot_rad_pts*n_leb
        !npts = matrix_size

        WRITE(*,*) 'Constructing molecular grid of',matrix_size,'points...'
...
! weight; and the density. We follow closely the scheme in the Becke paper.
        j=0
        counter = 0
        do atom_b=1,num_atoms,1

                ALLOCATE(r_weights(nr_rad_pts(centre_charge(atom_b))))
                ALLOCATE(r_points(nr_rad_pts(centre_charge(atom_b))))
                WRITE(*,*) "....for atom....",atom_b
! Get all the radial points and the required weights
                call becke_grid(atom_b,nr_rad_pts(centre_charge(atom_b)),r_weights,r_points)

                do point_r=1,nr_rad_pts(centre_charge(atom_b)),1

                        do point_l=1,n_leb,1

! The Lebedev grid is on the unit sphere and centered at (0,0,0).
! So it needs to be shifted to a new center, and multiplied with the actual radius
                               call coord_point(atom_b,point_r,point_l)

                               call becke_weight(atom_b,b_weight)

! Store the grid data
! 1,2 en 3: the x,y and z coordinates
! 4: all grid integration weights EXCEPT the Hirshfeld weight
! 5: the molecular density computed at the grid point
                                !WRITE(39,*) 4*3.14159265*r_weights(point_r)*w_leb(point_l)*b_weight,&
                                !            x_coord,y_coord,z_coord
                                counter = counter+1
                                wght(counter)=4*3.14159265*r_weights(point_r)*w_leb(point_l)*b_weight
                                !wght(counter)=b_weight
                                rad_grid(counter,1)=x_coord
                                rad_grid(counter,2)=y_coord
                                rad_grid(counter,3)=z_coord
                                !print *,centre_coord(atom_b,1),x_coord
                        END do

                END do

                deallocate (r_weights)
                deallocate (r_points)
...
subroutine atom_primitives()

! This matrix links primitives to atoms
! prim_atom(X,1) tells what is the first primitive of centre X
! prim_atom(X,2) tells what is the last primitive of centre X
! Warning: this assumes your primitives are sequentially organized
! This subroutine is in fact used seldomly for Hirshfeld-I
...
centre = centre_assignm(1)
prim_atom(centre,1) = 1
do prim = 1,num_prim,1
	if (centre_assignm(prim) .ne. centre) then
        	prim_atom(centre,2) = prim-1
                centre = centre_assignm(prim)
        	prim_atom(centre,1) = prim
        end if

end do

prim_atom(centre,2) = num_prim

END subroutine

subroutine charge_density_matrix()

! Subroutine that creates the charge and bond order matrix
! Note that the description matrix is tricky, it is actually
! a vector

USE Pvariables
implicit none

INTEGER::i,j,k,mol_orb
...
do i=1,num_prim,1
	do j=1,i,1
		k=k+1
		do mol_orb=1,num_orb,1
			p_matrix(k) = p_matrix(k) + (occup_nr(mol_orb)&
        			&*mo_coeff(mol_orb,i)*mo_coeff(mol_orb,j))
		END do
	END do
END do
...
REAL*8,DIMENSION(95)::BrSl_radii
...
subroutine ang_momentum_matrix()
...
type_exponents(1,2)=0.0
type_exponents(1,3)=0.0

type_exponents(2,1)=1.0
type_exponents(2,2)=0.0
type_exponents(2,3)=0.0

type_exponents(3,1)=0.0
type_exponents(3,2)=1.0
type_exponents(3,3)=0.0

type_exponents(4,1)=0.0
type_exponents(4,2)=0.0
type_exponents(4,3)=1.0

type_exponents(5,1)=2.0
type_exponents(5,2)=0.0
type_exponents(5,3)=0.0

type_exponents(6,1)=0.0
type_exponents(6,2)=2.0
type_exponents(6,3)=0.0

type_exponents(7,1)=0.0
type_exponents(7,2)=0.0
type_exponents(7,3)=2.0

type_exponents(8,1)=1.0
type_exponents(8,2)=1.0
type_exponents(8,3)=0.0

type_exponents(9,1)=1.0
type_exponents(9,2)=0.0
type_exponents(9,3)=1.0

type_exponents(10,1)=0.0
type_exponents(10,2)=1.0
type_exponents(10,3)=1.0

type_exponents(11,1)=3.0
type_exponents(11,2)=0.0
type_exponents(11,3)=0.0

type_exponents(12,1)=0.0
type_exponents(12,2)=3.0
type_exponents(12,3)=0.0

type_exponents(13,1)=0.0
type_exponents(13,2)=0.0
type_exponents(13,3)=3.0

type_exponents(14,1)=2.0
type_exponents(14,2)=1.0
type_exponents(14,3)=0.0

type_exponents(15,1)=2.0
type_exponents(15,2)=0.0
type_exponents(15,3)=1.0

type_exponents(16,1)=0.0
type_exponents(16,2)=2.0
type_exponents(16,3)=1.0

type_exponents(17,1)=1.0
type_exponents(17,2)=2.0
type_exponents(17,3)=0.0

type_exponents(18,1)=1.0
type_exponents(18,2)=0.0
type_exponents(18,3)=2.0

type_exponents(19,1)=0.0
type_exponents(19,2)=1.0
type_exponents(19,3)=2.0

type_exponents(20,1)=1.0
type_exponents(20,2)=1.0
type_exponents(20,3)=1.0

end subroutine

subroutine nr_radial_points(nr_points)

! Establish the number of radial points for each atom
! Becke recipe
! See start of the code for the meaning of radfac

USE Pvariables
implicit none

INTEGER::i
INTEGER,DIMENSION(118)::nr_points
...
subroutine becke_grid(atom,nr_pts,radial_weights,radial_points)
...
do j = 1,nr_pts,1
        ...
        i=REAL(j)
        x_point = COS((i*pi)/(1.0+nr_pts))
        radial_points(i) = ((1.0+x_point)/(1.0-x_point))*(BraggSlater_radii(centre_charge(atom))/2.0)
        radial_weights(i) = ((2.0*pi)/(1.0+nr_pts))*(((1.0+x_point)**(2.5))/((1.0-x_point)**(3.5)))&
        			&*(BraggSlater_radii(centre_charge(atom))/2.0)**3.0

end do

end subroutine

subroutine coord_point(atom,pt_r,pt_l)

! Transforms the coordinates from the unit sphere on the Lebedev grid

USE Pvariables
implicit none

INTEGER::atom,pt_r,pt_l

x_coord = (x_leb(pt_l) * r_points(pt_r)) + centre_coord(atom,1)
y_coord = (y_leb(pt_l) * r_points(pt_r)) + centre_coord(atom,2)
z_coord = (z_leb(pt_l) * r_points(pt_r)) + centre_coord(atom,3)

end subroutine

subroutine becke_weight(atom,weight)
...
INTEGER::atom,atom1
REAL*8::weight,cell,sum_weights

sum_weights = 0.0d0

do atom1=1,num_atoms,1
	call voronoi(atom1,cell)
...
subroutine voronoi(atom_one,Vcell)
...
INTEGER::atom_one,atom2
REAL*8::stepftion,Vcell

Vcell = 1.0d0
atom2 = 1

do WHILE((atom2 <= num_atoms) .AND. (Vcell > 10.0**(-10.0)))
        IF(atom2 .ne. atom_one) THEN

                call step_function(atom_one,atom2,stepftion)
...
subroutine step_function(atom_een,atom_twee,stepfunction)
...
INTEGER::atom_een,atom_twee,i
REAL*8::ellipt_coord_hom,radii_ratio,coeff,dist_1,dist_2
REAL*8::becke_coeff,ellipt_coord_het,stepfunction
REAL*8::internucl_dist

dist_1 = SQRT((x_coord-centre_coord(atom_een,1))**2.0+(y_coord-centre_coord(atom_een,2))**2.0&
		&+(z_coord-centre_coord(atom_een,3))**2.0)
dist_2 = SQRT((x_coord-centre_coord(atom_twee,1))**2.0+(y_coord-centre_coord(atom_twee,2))**2.0&
		&+(z_coord-centre_coord(atom_twee,3))**2.0)
internucl_dist = SQRT((centre_coord(atom_twee,1)-centre_coord(atom_een,1))**2.0&
	&+(centre_coord(atom_twee,2)-centre_coord(atom_een,2))**2.0&
	&+(centre_coord(atom_twee,3)-centre_coord(atom_een,3))**2.0)

ellipt_coord_hom = (dist_1 - dist_2) / internucl_dist


IF((centre_charge(atom_een).EQ.1.0) .AND. (centre_charge(atom_twee).EQ.1.0)) then
        radii_ratio = 1.0
Else IF(centre_charge(atom_een).EQ.1.0) then
        radii_ratio = 0.7/BraggSlater_radii(centre_charge(atom_twee))
ELSE IF(centre_charge(atom_twee).EQ.1.0) then
        radii_ratio = BraggSlater_radii(centre_charge(atom_een))/0.7
Else
	radii_ratio = BraggSlater_radii(centre_charge(atom_een))/BraggSlater_radii(centre_charge(atom_twee))
END if
...
!*******************************************************************************!
!-------------------------------------------------------------------------------!
! SUBROUTINE TO BE CALLED FROM GEM

subroutine wfnBECKE(rad_grid,wght,npts,maxpts,radnum,angnum,wfnfilename)

USE kinds; USE Patrick; USE Pvariables

IMPLICIT NONE

INTEGER   :: i,j,npts,maxpts,radnum,angnum
double precision :: rad_grid(maxpts,3),wght(maxpts)
CHARACTER(30) :: wfnfilename!,Beckefilename,wfnfilename

!Initialize variables for the grid.
stepf_iter = 3

!Read *.wfn file.
call read_data(wfnfilename)

!Loop over all grids
radfac=radnum
n_leb=angnum
!print *,'before prepare ',radfac,n_leb
call prepare(rad_grid,wght,npts,maxpts)
!print *,'after prepare'



END SUBROUTINE wfnBECKE
