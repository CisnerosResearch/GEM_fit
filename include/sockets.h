 ...
 */
#ifndef SIMPLE_SOCKETS_H
#define SIMPLE_SOCKETS_H
...
# ifndef __USE_BSD
  ...
  /* Service name is 'sslp'. Should do a getservbyname("sslp","tcp") in order
   * to honor /etc/services. We could do this, at least in UNIX:
   #include <netdb.h>
   #define PORTMASTER (getservbyname("sslp","tcp")?ntohs(getservbyname("sslp","tcp")->s_port):(u_short)1750)
   * Does a double lookup, but works well.
   */
